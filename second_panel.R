...
main_power_second_stats1 <- main_power_main_panel1 %>%
  group_by(date) %>%
  summarize(count = n(),
            min_watts = min(Value),
            mean_watts = mean(Value),
            max_watts = max(Value))
main_power_second_stats2 <- main_power_main_panel2 %>%
  group_by(date) %>%
  summarize(count = n(),
            min_watts = min(Value),
            mean_watts = mean(Value),
...
cor(main_power_second_stats$mean_watts, main_power_second_stats$mean_temp) #-0.71
...
main_power_second_stats %>%
  ...
  labs(x = "Average Temperature",
       ...
       title = "Second Panel Watts and Temp.",
       subtitle = "Correlation coefficient of -0.71") +
  theme_minimal() +
  theme_tej() +
  transition_reveal(mean_temp)
